...
class Temp extends Component {
  ...
  async componentDidMount() {
    ...
    for (let i = 0; i < banglaQ.length; i++) {
      ...
      for (let j = 0; j < englishQ[i].length; j++) {
        let eq = englishQ[i][j]
        equestion.push(eq);
      }
    }
    console.log("length",'ban',bquestion.length,'eng',equestion.length);
    this.adds = add
          ...
          this.state.level >= this.state.questions.length ? this.modal() :
            this.state.language && this.state.level >= 0 ?
              <div style={{
                ...
                {/* </div> */}
                <text style={{color: 'white',position:"absolute",top:10,right:10}}>{this.state.level}</text>
              </div>
              : this.languageScreen()}
      </div>
